...
# UNTESTED: Fedora build + run helper for Elite: Dangerous Colonisation Assistant.
#
# This is a convenience wrapper around the same workflow as
# [`run-edca-built-debian.sh`](run-edca-built-debian.sh:1), but with Fedora-specific hints.
#
# Fedora prerequisite hints (not performed automatically):
#   - Install base tools:
...
#
# Usage:
#   chmod +x ./run-edca-built-fedora.sh
#   EDCA_PYTHON=python3.12 EDCA_VENV_DIR=.venv312 ./run-edca-built-fedora.sh
#
...
if [ "${PY_MAJOR_MINOR}" = "3.13" ]; then
  log "NOTE: Python 3.13 detected. If you hit pydantic-core build errors, use Python 3.12 via uv and run with: EDCA_PYTHON=python3.12 EDCA_VENV_DIR=.venv312 ./run-edca-built-fedora.sh"
fi
