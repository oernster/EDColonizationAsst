...
class TrayController:
    ...
    def _start_backend(self) -> Optional[ProcessGroup]:
        ...
        self._log_message(f"Starting backend process: {' '.join(cmd)} (cwd={self._root})")
    ...
    def _start_frontend(self) -> Optional[ProcessGroup]:
        ...
        self._log_message(f"Starting frontend process: {' '.join(cmd)} (cwd={frontend_dir})")
        return self._spawn_process(cmd, cwd=frontend_dir, name="frontend")
 
    def _spawn_process(self, cmd: list[str], cwd: Path, name: str) -> Optional[ProcessGroup]:
        """
