...
class ConstructionSite(BaseModel):
    ...
    construction_progress: float = Field(ge=0.0, le=100.0, description="Overall construction progress")
    ...
    commodities: List[Commodity] = Field(default_factory=list, description="Required commodities")
    last_updated: datetime = Field(default_factory=lambda: datetime.now(UTC), description="Last update timestamp")
    last_source: DataSource = Field(default=DataSource.JOURNAL, description="Source of the last update")
...
class SystemColonizationData(BaseModel):
    ...
    construction_sites: List[ConstructionSite] = Field(
        ...
        description="All construction sites in the system"
    )
...
class CommodityAggregate(BaseModel):
    ...
    total_required: int = Field(ge=0, description="Total amount required across all sites")
    total_provided: int = Field(ge=0, description="Total amount provided across all sites")
