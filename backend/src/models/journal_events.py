...
class ColonizationConstructionDepotEvent(JournalEvent):
    ...
    construction_complete: bool = Field(default=False, description="Construction complete flag")
    construction_failed: bool = Field(default=False, description="Construction failed flag")
    commodities: List[Dict[str, Any]] = Field(
        default_factory=list,
        description="List of required commodities"
    )
...
class ColonizationContributionEvent(JournalEvent):
    ...
    commodity_localised: Optional[str] = Field(None, description="Localized commodity name")
...
class LocationEvent(JournalEvent):
    ...
    system_address: int = Field(description="System address")
    star_pos: List[float] = Field(default_factory=list, description="Star position coordinates")
